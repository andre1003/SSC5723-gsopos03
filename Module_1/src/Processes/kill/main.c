...
int main(int argc, char const *argv[]) {
    ...
    if(id == 0){
        /* This code is executed by the SON process
           Some information that identifies this is displayed on the console, such as: SON PID and FATHER PID */
        printf("  [SON]: My PID is -> %i\n", getpid());
        printf("  [SON]: My father's PID is -> %i\n", getppid());

        // Display in the console the print that indicates the infinite loop implemented to keep the SON process always running (unless it is forced to terminate)
    ...
    // If the ID returned is GREATER than zero (0), then we are in the FATHER process
        ...
        // Terminates the SON process by sending a "-9" signal and waits to move on
        kill(id, SIGKILL);
        wait(NULL);
