...
int main(int argc, char const *argv[]) {
    pid_t id; // Variable that will receive the return ID of fork() on creation of the new process
    int sec = 3; // The "sec" is to sleep some seconds and monitor the executions on the console
    int num = 10; // The "num" is to illustrate that the new process is a copy of the father, but changing the value by the son does not affect the value of the father
    ...
    if(id > 0) {
        /* This is the code that is executed by the FATHER process
           Some information that identifies this is displayed on the console, such as: FATHER PID, address space used by "num" value and your value */
        ...
        /* "wait()" is also a system call primitive
            Although the focus is on illustrating fork(), it was used here to block the father process execution until the son terminates */
        wait(NULL);
    ...
    // If the ID returned is EQUAL to zero (0), then we are in the SON process
        ...
        /* This code is executed by the SON process
           Some information that identifies this is displayed on the console, such as: SON PID, FATHER PID, address space used when handle "num" and your value after manipulation */
        printf("  [SON]: My PID is -> %i\n", getpid());
        printf("  [SON]: My father's PID is -> %i\n", getppid());

        /* Sum performed to change value of "num" by the SON process
           Even using the same address space, this manipulation should not affect the value of "num" being processed in the FATHER process */
        num += 10;
    ...
    // If the ID returned is different from anything (for example, SMALLER than zero (0)), so it was not possible to execute fork() and the new (son) process was not created
    else {
